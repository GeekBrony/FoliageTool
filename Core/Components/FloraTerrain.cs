    ...
    public static IEnumerator Refresh(FloraTerrain component)
    {
        ...
        for (int x = 0; x < divisions; x++)
        {
            ...
            for (int y = 0; y < divisions; y++)
            {
                ...
                EditorUtility.DisplayProgressBar("Planting foliage", $"Refreshing terrain \"{component.name}\" segment {numTerrains+1}/{terrainCount}", progress);
#endif
        ...
        for (int y = 0; y < detailRegion.height; ++y)
        {
            for (int x = 0; x < detailRegion.width; ++x)
            {
                ...
                foreach (TreeInstance tree in trees)
                {
                    ...
                    float dist = treeSize - math.distance(new float2(treePos.z, treePos.x), positions.WorldPosition2D);
                    float step = math.smoothstep(treeBlendRange.x, treeBlendRange.y, math.saturate(dist / treeSize));
